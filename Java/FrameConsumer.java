...
public class FrameConsumer implements Runnable{
		...
		while(true){
			try {
				if((int) VisionTable.getNumber("Camera To Use", 0) == 0) {
					...
					if(NetworkTable.getTable("LiveWindow").getBoolean("Tets Mode Vision", false)){
					}
					...
					if(bound.size() == 2) {		
						...
						while (countoursRun < bound.size() - 1) {
							...
							if(isGood(contours.get(countoursRun), contours.get(countoursRun + 1))) {
								//taking the left point and the right point of the contour that he fond
	...
	public double rotaiton(MatOfPoint2f points, boolean scale) {
		...
		for (int i = 0; i < 4; i++) {
			try {
				Imgproc.line(this.queue.take().getMat(), point[i], point[(i+1)], new Scalar(0,0,255));
	...
	public void sendValuesToRobot(double[] x, double[] y, double[] target_w, long time) {
		...
		if(time == 0) {
			NetworkTable.getTable("DriverPanel").putBoolean("Vision detect", false);
		} else {
			NetworkTable.getTable("DriverPanel").putBoolean("Vision detect", true);
		}
	...
	public boolean isGood(MatOfPoint rightContourPoints, MatOfPoint leftContourPoints){
		...
		int[] leftXSort = sortFromSmallToBig(leftXPoints);
		int[] leftYSort = sortFromSmallToBig(leftYPoints);
		int[] rightXSort = sortFromSmallToBig(rightXPoints);
		int[] rightYSort = sortFromSmallToBig(rightYPoints);
		...
		Point[] leftPointSort = sortPoints(leftPoints, leftYSort, leftXSort);
		Point[] rightPointSort = sortPoints(rightPoints, rightYSort, rightXSort);

		return rightPointSort[3].x - leftPointSort[0].x < rightPointSort[2].x - leftPointSort[1].x;
	}


	public int[] sortFromSmallToBig(double[] points){

		int[] smallToBig = new int[points.length];
	...
	public Point[] sortPoints(Point[] points, int[] smallToBigY, int[] smallToBigX){
		Point[] sort = new Point[4];
